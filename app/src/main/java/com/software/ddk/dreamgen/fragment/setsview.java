...
public class setsview extends Fragment {
    ...
    private PopupWindow pop_setname, pop_setinfo, pop_waifuedit;
    private RecyclerView rv_sets;
    private RVWaifuItems waifuadapter;

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup parent, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_setsview, parent, false);
    }
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        fab_addset = view.findViewById(R.id.fab_addset);
        rv_sets = view.findViewById(R.id.rv_sets);

        //preparar datos de sets
        SaveManager smg = new SaveManager(getContext(), SaveManager.SAVETYPE_WAIFUDATA);
        sets_waifus = smg.getArray("waifusets");
        load_data(0);

        //poblar recyclerview
        LinearLayoutManager waifuLayout = new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, true);
        waifuLayout.setStackFromEnd(true);
        rv_sets.setLayoutManager(waifuLayout);
        waifuadapter = new RVWaifuItems(getContext(), sets_waifus);
        rv_sets.setAdapter(waifuadapter);


        rv_sets.addOnItemTouchListener(new RecyclerItemClickListener(getContext(), rv_sets, new RecyclerItemClickListener.OnItemClickListener() {
            @Override
    ...
    private void open_pop_addset(View view) {
        ...
        try {
            ...
            View layout = inflater.inflate(R.layout.layout_pop_newset, (ViewGroup) view.findViewById(R.id.pop_newset));
    ...
    private void pop_open_setinfo(View view, final int position) {
        LayoutInflater inflater = (LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View layout = inflater.inflate(R.layout.layout_pop_waifu_item, (ViewGroup) view.findViewById(R.id.pop_waifu_item));

        int width = LinearLayout.LayoutParams.MATCH_PARENT;
        int height = Math.round(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 340, getResources().getDisplayMetrics()));
        ...
        LinearLayout bt_conf_accept, bt_conf_delete, bt_conf_close, bt_conf_edit;
        ...
        bt_conf_edit = layout.findViewById(R.id.bt_conf_edit);
        ...
        if (position == 0){
            ...
            if (sets_waifus.get(position).length > 41){
                if (sets_waifus.get(position)[41].equals("1")){
        ...
        bt_conf_edit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                pop_waifu_edit(view, position);
                pop_setinfo.dismiss();
            }
        });
        ...
        iv_fav.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String[] data = sets_waifus.get(position);
    ...
    private void pop_waifu_edit(View view, final int position){
        try {
            LayoutInflater inflater = (LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            View layout = inflater.inflate(R.layout.layout_pop_waifu_edit_item, (ViewGroup) view.findViewById(R.id.cl_pop_waifu_edit_item));
            ...
            pop_waifuedit = new PopupWindow(layout, width, height, true);
            pop_waifuedit.setContentView(layout);
            pop_waifuedit.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
            pop_waifuedit.showAtLocation(getView(), Gravity.CENTER, 0, 0);
            ...
            final String[] data = sets_waifus.get(position);
            et_edit_name.setText(data[32]);
            et_edit_desc.setText(data[38]);

            bt_edit_accept.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    //guardar
                    data[32] = et_edit_name.getText().toString();
                    data[38] = et_edit_desc.getText().toString();
                    data[40] = getSaveDate();

                    save_set_edit_data(position, data);
                    pop_waifuedit.dismiss();
                }
            });
    ...
    private void save_set_edit_data(int position, String[] data){
        sets_waifus.set(position, data);
        SaveManager smg = new SaveManager(getContext(), SaveManager.SAVETYPE_WAIFUDATA);
        smg.saveArray("waifusets", sets_waifus);
        new CustomToast(getContext(), "Conjunto guardado.", Toast.LENGTH_SHORT, Gravity.CENTER);
        waifuadapter.notifyDataSetChanged();
    }
    ...
    private void delete_set(int position) {
        if (position > 0){
            SaveManager smg = new SaveManager(getContext(), 2);
            sets_waifus.remove(position);
            smg.saveArray("waifusets", sets_waifus);
            update_adapter();
            new CustomToast(getContext(), "Conjunto eliminado.", Toast.LENGTH_SHORT, Gravity.BOTTOM);
        } else {
            new CustomToast(getContext(), "No puedes eliminar este set.", Toast.LENGTH_SHORT, Gravity.BOTTOM);
        }
    }
    ...
    private void addset(View view, String setname, String setdesc) {
        ...
        smg.saveArray("waifusets", sets_waifus);
        new CustomToast(getContext(), "Conjunto guardado.", Toast.LENGTH_SHORT, Gravity.BOTTOM);
